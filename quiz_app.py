#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""SAP-C02 Practice Quiz – Offline CLI
Generated from your PracticeTest.txt by ChatGPT.
This script is standalone: it embeds all questions and answers below.
Usage:
  python quiz_app.py
Controls:
  - Enter letters for your answer, e.g. A or AC (for multi-select)
  - Commands: skip, back, help, quit
...
def wrap(s):
    return "\n".join(textwrap.wrap(s, WRAP)) if s else ""

def print_header(i, total, qid):
    print("\n" + "="*WRAP)
    print(f"Question {i+1}/{total}  (ID {qid})")
    print("-"*WRAP)
...
def show_question(q):
    ...
    if q['select_n'] > 1:
        print(f"\nSelect exactly {q['select_n']} option(s).")

def norm(ans):
    return "".join(sorted(set(ans.upper())))
...
def parse_input(s):
    ...
    if s.lower() in ('help','h','?'):
        return ('cmd','help')
    if s.lower() in ('skip','s'):
        return ('cmd','skip')
    if s.lower() in ('back','b'):
        return ('cmd','back')
    # answers: letters only
    letters = "".join([c for c in s.upper() if c.isalpha()])
    return ('ans', norm(letters))

def check_answer(q, user_letters):
    # validate allowed choices
    allowed = set(q['options'].keys())
    if any(c not in allowed for c in user_letters):
        bad = [c for c in user_letters if c not in allowed]
        return False, f"Invalid choice(s): {', '.join(bad)}"
    # selection count enforcement
    if len(user_letters) != q['select_n']:
        return False, f"This question expects {q['select_n']} selection(s), but you entered {len(user_letters)}."
    correct = norm("".join(q['correct']))
    return (user_letters == correct), correct

def print_help():
    print("""
Commands:
  - Enter letters for your answer, e.g. 'A' or 'AC' (no spaces) for multi-select.
  - 'skip' to move to the next question.
  - 'back' to go to the previous question.
  - 'help' to show this message.
  - 'quit' to exit.
Tips:
  - Your input must have exactly the number of selections required when prompted.
  - Case-insensitive; 'ac' = 'AC'.
""")
...
def main():
    ...
    total = len(qs)
    if total == 0:
        print("No questions found in the embedded dataset. Exiting.")
        return 1

    # Optional shuffle prompt
    try:
        choice = input("Shuffle questions? [Y/n]: ").strip().lower()
    except EOFError:
        choice = 'y'
    if choice in ('', 'y', 'yes'):
        random.shuffle(qs)
    ...
    while 0 <= idx < total:
        q = qs[idx]
        print_header(idx, total, q['id'])
        show_question(q)
        try:
            raw = input("\nYour answer (letters or command): ")
        except EOFError:
        ...
        if kind == 'cmd':
            ...
            user_letters = val
            ok, extra = check_answer(q, user_letters)
            if ok:
                print("✅ Correct!")
                score += 1
                answered += 1
                if q.get('explanation'):
                    print("\nExplanation:")
                    print(wrap(q['explanation']))
                idx += 1
            else:
                # If 'extra' is the correct answer letters, show formatted; else show error.
                if len(extra) <= 10 and extra.isalpha():
                    print(f"❌ Incorrect. Correct answer: {' '.join(list(extra))}")
                else:
                    print(f"⚠️  {extra}")
                if q.get('explanation'):
                    print("\nExplanation:")
                    print(wrap(q['explanation']))
                answered += 1
                # After showing feedback, proceed to next
                idx += 1

    print("\n" + "="*WRAP)
    print(f"Final: {score} correct out of {answered} attempted ({(score/max(1,answered))*100:.1f}%).")
