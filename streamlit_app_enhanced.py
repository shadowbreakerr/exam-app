...
QUESTIONS = [
  {
    "id": "1",
    "question": "- (Exam Topic 1) A solutions architect is designing a publicly accessible web application that is on an Amazon CloudFront distribution with an Amazon S3 website endpoint as the origin. When the solution is deployed, the website returns an Error 403: Access Denied message. Which steps should the solutions architect take to correct the issue? (Select TWO.)",
    "options": {
...
"D": "Replace the DB instance with Amazon Aurora with Aurora Replicas. Deploy the application to multiple smaller EC2 instances across multiple Availability Zones in an Auto Scaling group behind an ALB."
  ...
  {
    "id": "6",
    "question": "- (Exam Topic 1) A company has developed an application that is running Windows Server on VMware vSphere VMs that the company hosts on premises. The application data is stored in a proprietary format that must be read through the application. The company manually provisioned the servers and the application. As part of its disaster recovery plan, the company wants the ability to host its application on AWS temporarily if the company's on-premises environment becomes unavailable. The company wants the application to return to on-premises hosting after a disaster recovery event is complete. The RPO is 5 minutes. Which solution meets these requirements with the LEAST amount of operational overhead?",
    "options": {
...
"D": "Configure AWS Transfer for FTP. Configure the FTP client with credentials. Script the client to connect and sync to start the transfer."
  ...
  {
    "id": "8",
    "question": "- (Exam Topic 1) A company has a three-tier application running on AWS with a web server, an application server, and an Amazon RDS MySQL DB instance. A solutions architect is designing a disaster recovery (DR) solution with an RPO of 5 minutes. Which solution will meet the company's requirements?",
    "options": {
...
"D": "Create AMIs of the web and application servers in the DR Region. Use scheduled AWS Glue jobs to synchronize the DB instance with another DB instance in the DR Region. In the event of a disaster, switch to the DB instance in the DR Region and reprovision the servers with AWS CloudFormation using the AMIs."
    ...
    "select_n": 1,
    "explanation": "A cross-Region RDS MySQL read replica provides near-real-time async replication, easily meeting a 5-minute RPO. In a disaster you promote the replica, then spin up web/app tiers from AMIs (via CloudFormation)",
    "tags": [
      "Analytics/Big Data",
      "Compute/EC2",
      "Databases",
...
"D": "Create an AWS WAF web ACL with an IP set match rule, and set the rule action to Block. Connect the web ACL to the ALB."
  ...
  {
    "id": "53",
    "question": "- (Exam Topic 1) A company stores sales transaction data in Amazon DynamoDB tables. To detect anomalous behaviors and respond quickly, all changes to the items stored in the DynamoDB tables must be logged within 30 minutes. Which solution meets the requirements?",
    "options": {
...
"D": "Create an IAM role in each account in the organization. Grant permissions to update security groups. Deploy an AWS Lambda function in the security team's AWS account. Configure the Lambda function to take a list of internal IP addresses as input, assume a role in each organization account, and add the list of IP addresses to the security groups in each account."
  ...
  {
    "id": "197",
    "question": "- (Exam Topic 2) A company plans to refactor a monolithic application into a modern application designed deployed or AWS. The CLCD pipeline needs to be upgraded to support the modem design for the application with the following requirements\n• It should allow changes to be released several times every hour. * It should be able to roll back the changes as quickly as possible Which design will meet these requirements? A. Deploy a Cl-CD pipeline that incorporates AMIs to contain the application and their configurations Deploy the application by replacing Amazon EC2 instances B. Specify AWS Elastic Beanstak to sage in a secondary environment as the deployment target for the CI/CD pipeline of the applicatio C. To deploy swap the staging and production environment URLs. D. Use AWS Systems Manager to re-provision the infrastructure for each deployment Update the AmazonEC2 user data to pull the latest code art-fact from Amazon S3 and use Amazon Route 53 weighted routing to point to the new environment E. Roll out At application updates as pan of an Auto Scaling event using prebuilt AMI F. Use new versions of the AMIs to add instances, and phase out all instances that use the previous AMI version with the configured termination policy during a deployment event.",
    "options": {
...
def init_state():
    ...
    if "timer_secs" not in st.session_state:
        st.session_state.timer_secs = 0
    if "per_q_limit" not in st.session_state:
        st.session_state.per_q_limit = 0
...
st.caption("Filters, bookmarks, timed mode, and review of incorrect answers.")
...
with st.sidebar:
    st.header("Controls")
    # Filters
    chosen_tags = st.multiselect("Filter by tag(s)", options=all_tags, default=[])
    only_unanswered = st.checkbox("Only unanswered", value=False)
    st.markdown("---")
    shuffle = st.checkbox("Shuffle questions", value=True)
    st.markdown("**Timed mode**")
    per_q = st.number_input("Seconds per question (0 = off)", min_value=0, max_value=600, value=0, step=5)
    if st.button("Start/Reset"):
        st.session_state.per_q_limit = int(per_q)
        st.session_state.order = [i for i in range(len(QUESTIONS)) if (not chosen_tags or any(t in QUESTIONS[i]['tags'] for t in chosen_tags))]
        if only_unanswered:
            st.session_state.order = [i for i in st.session_state.order if i not in st.session_state.answered]
        if shuffle:
        ...
        st.session_state.revealed = {}
        st.session_state.review_incorrect = False
        st.session_state.start_time = time.time() if st.session_state.per_q_limit > 0 else None
...
qi, q = current_q()
if q is None:
    st.warning("No questions match your current filter. Adjust filters and press Start/Reset.")
    st.stop()

wrap_width = 110
st.subheader(f"Question {st.session_state.idx+1} / {len(st.session_state.order)}  (ID {q['id']})")
st.caption("Tags: " + ", ".join(q.get("tags", [])))
st.write("\n".join(textwrap.wrap(q['question'], width=wrap_width)))

# Timer logic
if st.session_state.per_q_limit > 0:
    if st.session_state.start_time is None:
        st.session_state.start_time = time.time()
    elapsed = int(time.time() - st.session_state.start_time)
    remaining = max(0, st.session_state.per_q_limit - elapsed)
    tcol1, tcol2 = st.columns([1,6])
    with tcol1:
        st.metric("Time left", f"{remaining}s")
    if remaining == 0 and st.session_state.revealed.get(qi) is not True:
        st.session_state.revealed[qi] = True
        st.warning("⏰ Time's up! Revealing the answer.")
...
if q['select_n'] == 1:
    ...
    sel = st.multiselect("Select exactly {q['select_n']}:", options=letters, format_func=lambda k: f"{k}. {q['options'][k]}", key=user_key)
    user_sel = list(sel)

# Action buttons
cols = st.columns(6)
with cols[0]:
    if st.button("Check", key=f"check_{qi}"):
...
with cols[1]:
    if st.button("Reveal", key=f"reveal_{qi}"):
        st.session_state.revealed[qi] = True
with cols[2]:
    if st.button("Bookmark", key=f"bookmark_{qi}"):
        st.session_state.bookmarks.add(qi)
        st.toast("Bookmarked")
with cols[3]:
    if st.button("Previous", key=f"prev_{qi}"):
        st.session_state.idx = max(0, st.session_state.idx - 1)
        if st.session_state.per_q_limit > 0:
            st.session_state.start_time = time.time()
with cols[4]:
    if st.button("Next", key=f"next_{qi}"):
        st.session_state.idx = min(len(st.session_state.order)-1, st.session_state.idx + 1)
        if st.session_state.per_q_limit > 0:
            st.session_state.start_time = time.time()
with cols[5]:
    if st.button("Review Incorrect", key=f"review_incorrect_{qi}"):
        # Build list of only incorrect questions
        wrong = [i for i, chosen in st.session_state.answered.items() if sorted(list(chosen)) != sorted(QUESTIONS[i]['correct'])]
        st.session_state.order = wrong or st.session_state.order
        st.session_state.idx = 0
...
with st.expander("Bookmarks"):
    if st.session_state.bookmarks:
        bmap = {b: QUESTIONS[b]['id'] for b in sorted(st.session_state.bookmarks)}
        st.write(", ".join(bmap.values()))
        jump = st.selectbox("Jump to bookmark:", options=sorted(st.session_state.bookmarks), format_func=lambda x: QUESTIONS[x]['id'])
        if st.button("Go"):
            st.session_state.idx = st.session_state.order.index(jump) if jump in st.session_state.order else 0
            if st.session_state.per_q_limit > 0:
                st.session_state.start_time = time.time()
    else:
...
st.caption("Tip: Use filters in the sidebar, and 'Review Incorrect' to focus your practice.")